...
function buildReferenceLockPrompt(imageCount) {
  const singleImageRule = imageCount === 1
    ? "用户只上传了 1 张照片时，请把这 1 张照片作为唯一视觉素材。可以将同一张照片裁切成远景、中景、特写或局部细节来完成分镜和排版，但不要生成新的照片、城市、人物、建筑、风景或场景。"
    : `用户上传了 ${imageCount} 张照片时，请只使用这些上传照片作为视觉素材。可以在这些照片之间做拼接、裁切、放大、局部取景、顺序重排和统一调色，但不要生成上传照片以外的新场景或陌生人物。`;

  return [
    "【参考图锁定规则，必须优先遵守】",
    "必须严格使用用户上传的照片作为主要视觉素材。不要把上传照片只当作风格参考，不要重新生成新的城市、人物、建筑、风景、室内空间或道具。",
    "允许的操作：裁切、放大、局部取景、调色、加颗粒、加暗角、加文字、加描边、加贴纸、加手绘线条、加纸张纹理、加边框、做排版和拼接。",
    "禁止的操作：替换原图主体、改变人物身份、凭空添加陌生人物、凭空更换地点、重画成完全不同的画面、把原图变成只保留氛围的新图。",
    "最终封面必须能明显看出来自用户上传的原始照片，保留原图中的主体、建筑/场景、光线、空间关系和关键物品。",
    singleImageRule
  ].join("\n");
}
...
function buildAiPrompt() {
  ...
  const referenceLockPrompt = buildReferenceLockPrompt(imageCount);
